...
public class ReceivePacket64IOTest {
    ...
    public void testGetSourceAddress64() throws Exception {
        ...
        ReceivePacket64IO frame = new ReceivePacket64IO();
        for (byte b : data) {
            frame.receive(b);
        }
    ...
    public void testGetFrameType() throws Exception {
        ReceivePacket64IO frame = new ReceivePacket64IO();
        assertThat(frame.getFrameType(), is(equalTo(ReceivePacket64IO.FRAME_TYPE)));
    }

    @Test
    public void testReceive() throws Exception {
        ReceivePacket64IO frame = new ReceivePacket64IO();
        assertThat(frame.getState(), is(equalTo(ReceivePacket64IO.State.ADDRESS64)));

        for (int i = 0; i < 8; ++i) {
            assertThat(frame.getState(), is(equalTo(ReceivePacket64IO.State.ADDRESS64)));
            frame.receive((byte) i);
        }
        assertThat(frame.getState(), is(equalTo(ReceivePacket64IO.State.RSSI)));

        frame.receive((byte) 0x28);
        assertThat(frame.getState(), is(equalTo(ReceivePacket64IO.State.OPTIONS)));

        frame.receive((byte) 0x06);
        assertThat(frame.getState(), is(equalTo(ReceivePacket64IO.State.RECEIVE_DATA)));
