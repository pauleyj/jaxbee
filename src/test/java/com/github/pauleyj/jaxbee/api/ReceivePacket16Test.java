...
public class ReceivePacket16Test {
    ...
    public void testGetSourceAddress16() throws Exception {
        ...
        ReceivePacket16 frame = new ReceivePacket16();
        for (byte b : data) {
            frame.receive(b);
        }
    ...
    public void testGetFrameType() throws Exception {
        ReceivePacket16 frame = new ReceivePacket16();
        assertThat(frame.getFrameType(), is(equalTo(ReceivePacket16.FRAME_TYPE)));
    }

    @Test
    public void testReceive() throws Exception {
        ReceivePacket16 frame = new ReceivePacket16();
        assertThat(frame.getState(), is(equalTo(ReceivePacket16.State.ADDRESS16)));

        for (int i = 0; i < 2; ++i) {
            assertThat(frame.getState(), is(equalTo(ReceivePacket16.State.ADDRESS16)));
            frame.receive((byte) i);
        }
        assertThat(frame.getState(), is(equalTo(ReceivePacket16.State.RSSI)));

        frame.receive((byte) 0x28);
        assertThat(frame.getState(), is(equalTo(ReceivePacket16.State.OPTIONS)));

        frame.receive((byte) 0x06);
        assertThat(frame.getState(), is(equalTo(ReceivePacket16.State.RECEIVE_DATA)));
